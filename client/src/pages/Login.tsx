...
const Login: React.FC = () => {
  ...
  // Handle incoming OAuth callback tokens / errors
  useEffect(() => {
    ...
    if (oauthError) {
      ...
      api
        ...
        .then((res) => {
          if (res.data.success) {
            dispatch(setUser({ user: res.data.data, token: oauthToken }));
            navigate("/");
          } else {
            setServerError("Could not complete social sign-in. Please try again.");
          }
        })
        .catch((err) => {
          setServerError(err.response?.data?.message || "Social login verification failed.");
        })
        .finally(() => {
